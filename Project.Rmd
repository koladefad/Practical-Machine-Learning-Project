...
```{r Corr Analysis, eval=FALSE}
corMatrix <- cor(TrainSet[, -54])
corrplot(corMatrix, order = "hclust" , type = "lower",tl.cex = 0.6)
```

The plot is not shown in this write up due to size restrictions on github. However, the result of the correlation plot shows the level of relationship between the variables. Variables with high positive correlation are indicated by a dark blue colour while those with high negative correlation are denoted by a dark red colour.
